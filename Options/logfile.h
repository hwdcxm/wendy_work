...
// 设置控制台宏2
#define  _DEBUGCONSOLE	 // wendy

// 设置文件名宏3
//#define  _SETFILENAME	 // wendy add
...
{    
    ...
    LPCSTR cstime = "ntimesone";
	static int ntimes = 0;
	if(_tcsicmp(lpFile,cstime)==0)
	{
	    //字符串相等
		ntimes ++;
		if (ntimes % nLine != 0)
		...
		else if (_tcsicmp(lpFile,cstime)==0)
		{
			//当nLine==0 时,即CLogout_ntimesone(100,"xxxx")形式打印信息
			sprintf(szMsg, "[%02d:%02d:%02d.%03d ntimes=%d]:", 
				st.wHour, st.wMinute, st.wSecond, 
									st.wMilliseconds,ntimes);
...
//日志输出接口函数4  使用于CLogout_ntimesone(100,"xxxx")形式
static void CLogout_ntimesone(int nTime,LPCSTR lpFormat, ...)
{
	const DWORD BufSize = 2048;
    char szMsg[BufSize];
	...
	//输出有文件名及行号的消息
	CLog::logout("ntimesone", nTime,szMsg);
}
